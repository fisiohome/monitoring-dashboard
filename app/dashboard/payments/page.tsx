...
import { Loader2, Search, Filter, ChevronLeft, ChevronRight, BarChart2, List } from "lucide-react";
        ...
        try {
            ...
            if (searchQuery) {
                ...
                filtered = data.filter((p: any) =>
                    (p.patient?.name || p.user?.email || "").toLowerCase().includes(lower) ||
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold tracking-tight text-[#6200EE]">Payments</h1>

                <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
                    <button
                        onClick={() => setViewMode("LIST")}
                        className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-all ${viewMode === "LIST"
                    ...
                    <button
                        onClick={async () => {
                            ...
                            if (allPayments.length === 0) {
                                ...
                                try {
                                    ...
                                    const allData = await fetchAllPages(fetchPayments, { payment_status: "PAID" }, "bookings");
                        ...
                        className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-all ${viewMode === "REPORTS"
                            ? "bg-white text-[#6200EE] shadow-sm"
                            : "text-slate-500 hover:text-slate-700"
                            }`}
                    >
                ...
                <>
                    ...
                    <Card className="border-none shadow-sm rounded-3xl bg-white/50 backdrop-blur-sm">
                        <CardContent className="p-6">
                            <div className="flex flex-col md:flex-row gap-4">
                                ...
                                <Button variant="outline" className="border-slate-200 bg-white" onClick={() => {
                                    setSearchQuery("");
                                    setStatusFilter("ALL");
                                    setPage(1);
                                }}>
                    ...
                    <Card className="border-none shadow-sm rounded-3xl overflow-hidden bg-white">
                        <CardHeader className="bg-gradient-to-r from-purple-50 to-white border-b border-purple-100 px-6 py-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                            <CardTitle className="text-lg font-bold text-[#6200EE]">Transaction History</CardTitle>
                            <ExportExcelButton data={payments} fileName="Payments" onFetchAll={handleExportAll} />
                        </CardHeader>
                        <CardContent className="p-0">
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="border-b border-slate-200 hover:bg-transparent">
                                            <TableHead className="font-bold text-slate-700 text-xs uppercase tracking-wider py-4 px-6">Order ID</TableHead>
                                            <TableHead className="font-bold text-slate-700 text-xs uppercase tracking-wider py-4">Customer</TableHead>
                                            <TableHead className="font-bold text-slate-700 text-xs uppercase tracking-wider py-4">Amount</TableHead>
                                            <TableHead className="font-bold text-slate-700 text-xs uppercase tracking-wider py-4">Date</TableHead>
                                                ...
                                                >
                                                    ...
                                                    <TableCell className="font-medium text-slate-900 py-4">
                                                        <div className="flex items-center gap-2">
                                                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-100 to-purple-200 flex items-center justify-center text-xs font-bold text-[#6200EE]">
                                                                {(p.patient?.name || p.user?.email || "U").charAt(0).toUpperCase()}
                                                            </div>
                                                            <span>{p.patient?.name || p.user?.email || "Unknown"}</span>
                                                        </div>
                                                    </TableCell>
                                                    <TableCell className="text-slate-900 font-semibold py-4">
                                                        {p.total_amount ?
                                                            `Rp ${p.total_amount.toLocaleString('id-ID')}` :
                                                            p.total_price ? `Rp ${p.total_price.toLocaleString('id-ID')}` : "-"}
                                                    </TableCell>
                                                    ...
                                                    <TableCell className="py-4 text-right pr-6">
                                                        <Link href={`/dashboard/payments/${p.id}`}>
                                                            <Button variant="ghost" size="sm" className="h-8 text-slate-500 hover:text-[#6200EE] hover:bg-purple-50 font-medium">
                                                                View
                                                            </Button>
                                                        </Link>
                                                    </TableCell>
                            ...
                            <div className="flex items-center justify-between px-6 py-5 border-t border-slate-200 bg-gradient-to-r from-slate-50 to-white">
                                <div className="text-sm font-medium text-slate-600">
                                    Page <span className="font-bold text-[#6200EE]">{page}</span> of <span className="font-bold">{Math.ceil(totalItems / pageSize) || 1}</span>
                                </div>
