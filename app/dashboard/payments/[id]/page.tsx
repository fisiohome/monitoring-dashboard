...
import { ChevronLeft, Calendar, CreditCard, User, FileText, Clock, AlertCircle } from "lucide-react";
    ...
    if (error || !payment) {
        return (
            <div className="flex h-[50vh] w-full flex-col items-center justify-center gap-4">
                ...
                <p className="text-lg font-medium text-slate-900">{error || "Payment found"}</p>
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-slate-500 text-sm">
                        ID: <span className="font-mono text-[#6200EE]">{payment.registration_number || payment.id}</span>
                    </p>
                </div>
                ...
                <Card className="md:col-span-2 border-none shadow-sm rounded-3xl bg-white overflow-hidden">
                    ...
                    <CardContent className="p-6 space-y-6">
                        <div className="flex flex-col items-center justify-center py-6 bg-slate-50 rounded-2xl border border-slate-100 border-dashed">
                            <span className="text-slate-500 text-sm font-medium uppercase tracking-wider mb-1">Total Amount</span>
                        ...
                        <div className={`flex items-center justify-between p-4 rounded-xl border ${['PAID', 'SUCCESS'].includes(paymentStatus)
                            ...
                            <Badge className={`text-sm py-1 px-4 ${['PAID', 'SUCCESS'].includes(paymentStatus)
                                ...
                                {['PAID', 'SUCCESS'].includes(paymentStatus) ? "LUNAS" : "BELUM LUNAS"}
                            </Badge>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="space-y-1">
                                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Payment ID</span>
                            ...
                            <div className="space-y-1">
                                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Status History</span>
                        ...
                        <CardContent className="p-5">
                            <div className="flex items-center gap-4 mb-4">
                                <div className="h-12 w-12 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 font-bold text-lg">
                                    {(payment.patient?.name || payment.user?.email || "U").charAt(0).toUpperCase()}
                                </div>
                                <div>
                                    <p className="font-bold text-slate-900">{payment.patient?.name || payment.user?.name || "Unknown"}</p>
                                    <p className="text-sm text-slate-500">{payment.patient?.type || "Standard Customer"}</p>
                                </div>
                            </div>
                            <div className="space-y-3 pt-3 border-t border-slate-100">
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-slate-700">
                                        {payment.patient?.phone_number || payment.patient?.phone || payment.user?.phone_number || "-"}
                                    </p>
                    ...
                    <Card className="border-none shadow-sm rounded-3xl bg-white overflow-hidden">
                        ...
                        <CardContent className="p-5 space-y-4">
                            <div className="bg-blue-50 rounded-xl p-3 border border-blue-100">
                                <p className="text-xs text-blue-600 font-semibold uppercase mb-1">Service Type</p>
                                <p className="font-bold text-blue-900">{payment.service?.name || "Physiotherapy Session"}</p>
                            </div>
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="h-6 w-6 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden">
                                        {payment.therapist?.photo_url ? (
                                            <img src={payment.therapist.photo_url} alt="Therapist" className="h-full w-full object-cover" />
                                        ) : (
                                            <User className="h-3 w-3 text-slate-500" />
                                        )}
                                    </div>
                                    <p className="text-sm font-medium text-slate-700">{payment.therapist?.name || "Not assigned yet"}</p>
                                </div>
                            </div>

                            <div className="space-y-1">
                                <span className="text-xs text-slate-400 block">Appointment Schedule</span>
                                <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
                                    <Clock className="h-3.5 w-3.5 text-slate-400" />
                                    {payment.appointment_date ? format(new Date(payment.appointment_date), "PP p") : "Not scheduled"}
                                </div>
                            </div>
