        ...
        try {
            ...
            if (searchQuery) {
                ...
                filtered = bookings.filter((b: any) =>
                    (b.customer?.full_name || b.patient?.name || "").toLowerCase().includes(lower) ||
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold tracking-tight text-[#6200EE]">Order Status</h1>
            </div>
                            ...
                            <TableBody>
                                {loading ? (
                                    ...
                                    orders.map((order, index) => (
                                        ...
                                        >
                                            <TableCell className="font-mono text-xs font-semibold text-[#6200EE] py-4 px-6 cursor-pointer" onClick={() => window.location.href = `/dashboard/orders/${order.id}`}>
                                            ...
                                            <TableCell className="font-medium text-slate-900 py-4">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-100 to-purple-200 flex items-center justify-center text-xs font-bold text-[#6200EE]">
                                                        {(order.patient?.name || order.user?.email || "U").charAt(0).toUpperCase()}
                                                    </div>
                                                    <span>{order.patient?.name || order.user?.email || "Unknown"}</span>
                                                </div>
                                            </TableCell>
