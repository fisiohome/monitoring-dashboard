...
import { ArrowLeft, User, MapPin, Calendar, Activity, Package, Briefcase, Phone, Clock, ChevronRight } from "lucide-react";
...
export default function AppointmentDetailPage() {
    ...
    const { patient, address, service_name, package_name, status, appointment_date, visit_number, total_visits_in_booking } = appointment;

    // Calculate progress percentage
    const progress = total_visits_in_booking > 0
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 bg-gradient-to-r from-purple-50 to-white p-6 rounded-3xl border border-purple-100">
                    <div>
                        ...
                        <p className="text-slate-500 font-medium flex items-center gap-2">
                            <span className="font-mono text-xs bg-slate-100 px-2 py-1 rounded-md text-slate-600">{id}</span>
                        </p>
                    </div>
                    <div className="flex flex-col items-end gap-1 text-right">
                        <div className="text-sm text-slate-500 font-medium uppercase tracking-wide">Last Updated</div>
                        <div className="text-slate-900 font-medium">
                            {appointment.updated_at ? format(new Date(appointment.updated_at), "PPP p") : "-"}
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-6">
                                <div className="flex-1 space-y-4">
                                    <div className="grid grid-cols-2 gap-y-4 gap-x-8">
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Gender</p>
                                            <p className="text-base font-medium text-slate-900 capitalize">{patient?.gender?.toLowerCase()}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Age</p>
                                            <p className="text-base font-medium text-slate-900">{patient?.age} Years Old</p>
                                        </div>
                                        <div>
                                            <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Date of Birth</p>
                                            <p className="text-base font-medium text-slate-900">{patient?.date_of_birth}</p>
                                        </div>
                                    </div>
                    ...
                    <Card className="border-none shadow-sm rounded-3xl bg-white overflow-hidden group hover:shadow-md transition-shadow">
                        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <CardTitle className="text-lg font-bold text-slate-800">Location</CardTitle>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="p-6">
                ...
                <div className="space-y-8">
                    <Card className="border-none shadow-sm rounded-3xl bg-[#6200EE] text-white overflow-hidden relative">
                        ...
                        <CardHeader className="pb-2 relative z-10">
                            <CardTitle className="text-lg font-bold opacity-90">Service Details</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-6 relative z-10">
                            <div>
                                <p className="text-white/70 text-xs uppercase tracking-wider mb-1">Service Type</p>
                                <p className="text-2xl font-bold">{service_name?.replace(/_/g, " ")}</p>
                            </div>
                            <div>
                            ...
                            <div className="bg-white/10 rounded-2xl p-4 backdrop-blur-sm border border-white/10">
                                <div className="flex justify-between items-end mb-2">
                                    ...
                                    <span className="text-xl font-bold">{visit_number} <span className="text-sm font-normal text-white/60">/ {total_visits_in_booking}</span></span>
                                </div>
                    ...
                    <Card className="border-none shadow-sm rounded-3xl bg-white overflow-hidden">
                        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <CardTitle className="text-lg font-bold text-slate-800">Visit Schedule</CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent className="p-6">
                            <div className="flex items-start gap-4">
                                <div className="flex flex-col items-center justify-center bg-purple-50 rounded-2xl w-16 h-16 text-[#6200EE] border border-purple-100">
                                    <span className="text-xs font-bold uppercase">{appointment_date ? format(new Date(appointment_date), "MMM") : "-"}</span>
                                    <span className="text-xl font-bold">{appointment_date ? format(new Date(appointment_date), "d") : "-"}</span>
                                </div>
                                <div>
                                    <p className="text-sm text-slate-500 uppercase tracking-wide font-medium">Date & Time</p>
                                    <p className="text-lg font-semibold text-slate-900">
                                        {appointment_date ? format(new Date(appointment_date), "EEEE, MMMM d, yyyy") : "Not Scheduled"}
                                    </p>
                                    <div className="flex items-center gap-2 mt-1 text-slate-600 font-medium">
                                        <Clock className="h-4 w-4 text-[#6200EE]" />
                                        {appointment_date ? format(new Date(appointment_date), "h:mm a") : "--:--"}
                                    </div>
                                </div>
                            </div>
                        </CardContent>
